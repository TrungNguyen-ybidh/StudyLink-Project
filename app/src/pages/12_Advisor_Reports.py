...
#Load reports for this advisor
try:
    response = requests.get(f"{API_BASE}/{advisor_id}/reports", timeout=10)
    if response.status_code != 200:
        st.error(f"Failed to fetch reports: {response.status_code} {response.text}")
        st.stop()
...
student_id_input = st.number_input("Student ID", min_value=1, step=1, value=1)
...
if st.button("Create Report", use_container_width=True):
    if not title_input or not content_input:
        st.error("Title and content are required to create a report.")
    else:
        payload = {
            "title": title_input,
